...
uint32_t pm_alloc(){
    for(uint32_t i = pmbm_start; i < max_memory/4096; i++){
        if(!phys_bitmap[i]) return i * 4096;
    }
    return 0;
}
...
int32_t map(uint32_t paddr, void *vaddr, uint16_t attributes){
    uint32_t pd_index = (uint32_t)(long)vaddr >> 22 & 0x3ff;
    uint32_t pt_index = (uint32_t)(long)vaddr >> 12 & 0x3ff;
    
    pde_t *pd = (void *)0xfffff000;
    pte_t *pt = (void *)((uint64_t)0xffc00000 + (0x1000 * pd_index));
    if(!pd[pd_index].present){
        *(uint32_t *)(&pd[pd_index]) = 0;
        pd[pd_index].addr = pm_alloc() >> 12;
    ...
    *(uint32_t *)(&pt[pt_index]) = attributes;
